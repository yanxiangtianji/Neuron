function [J, grad] = costFunctionAWCore(X,y,adj,weight,     lambdaA=0,lambdaW=0, gdFctA=1,gdFctW=1, penA=0,penW=0,refA=0,refW=0)
...
%pen & ref: penalty parameter for A/W changes, and the referenced point of A/W value.
%           pen is a real scalar and ref is a vector (when pen is 0 ref is meaningless)
%           The larger the pen is, the closer the corresponding A/W is.
...
J+=lambdaA*sum(ea);
J+=penW/2*sumsq(weight-refW) + penA/2*sumsq(sa-refA);
